...
const fs = require("fs");
exports.getUnitDefects = asyncHandler(async (req, res, next) => {
  let query;
  if (req.params.unitId) {
    query = Defect.find({ unit: req.params.unitId });
  } else {
    query = Defect.find();
  }
...
exports.getDefect = asyncHandler(async (req, res, next) => {
...
exports.createDefect = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: "New Defect insert success.",
    user_id: req.userId,
    data: defect,
  });
});
exports.updateDefect = asyncHandler(async (req, res, next) => {
  const defect = await Defect.findById(req.params.id);
  if (!defect) {
    throw new LocalError(req.params.id + " ID is not include any Data.", 404);
  }
  if (defect.created_user !== req.userId && req.userRole !== "admin") {
    throw new LocalError("You just update your post. Not other's post", 404);
  }
  req.body.updated_user = req.userId;
  for (let attr in req.body) {
    (defect[attr] = req.body[attr]), { new: true, runValidators: true };
  }
  defect.save();
  res.status(200).json({
    success: true,
...
exports.deleteDefect = asyncHandler(async (req, res, next) => {
  const defect = await Defect.findById(req.params.id);
  if (!defect) {
...
exports.getUserDefect = asyncHandler(async (req, res, next) => {
  //Get Requist
  req.query.created_user = req.userId;
  const defect = await Defect.find(req.query);
  res.status(200).json({
    success: true,
    message: "Request success.",
    user_id: req.userId,
...
exports.uploadFilesDefect = asyncHandler(async (req, res, next) => {
  const defect = await Defect.findById(req.params.id);
  ...
  file.mv(`${process.env.FILE_UPLOAD_DEFECTS_PATH}/${file.name}`, (err) => {
    if (err) {
...
exports.imagesGetDefect = asyncHandler(async (req, res, next) => {
  try {
    const imageAll = await fs.readdirSync("./public/uploads/defects");
    res.status(200).json({
      success: true,
...
exports.generatePDF = asyncHandler(async (req, res, next) => {
  ...
  function buildPDF(dataCallback, endCallback) {
    ...
    doc.fontSize(12).text(`Reason : ${defect.reason}`);
    doc.fontSize(12).text(`Description: ${defect.description}`);
    doc.image(`public/uploads/defects/${defect?.photo?.[0].fileName}`, {
      fit: [250, 400],
      align: "center",
      valign: "center",
    });
...
exports.sendGmail = asyncHandler(async (req, res, nex) => {
  ...
  const link = `${process.env.MAIL_HOST}/api/v1/defects/${defect._id}/pdf`;
